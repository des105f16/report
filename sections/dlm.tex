...
This proves useful as it allows for label inference and label polymorphism (see \cref{dlm:implicit_labels} and ??). \mikkel{Insert reference when written.} \mikael{Are we going to write about label polymorphism?}\rene{Yes :)}
...
By having this relationship between labels, we can see the set of all our labels as a partially-ordered set: a \emph{lattice}.\renein{Lattice = partially ordered + structure}
...
Dual to the bottom element, we also have the most restrictive label: the top element, denoted $\top$.\renein{Reference til bevis/påstand for lattice egenskab}
