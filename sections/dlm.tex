...
\mikkel{Short description of principals hierarchies, runtime labels and what the difference is between runtime authority and the authority checks we can do at compile time.}
...
\mikael{Skriv om så vi benævner at labels kan anvendes alle steder der er type, da det er sådan DLM har defineret det.}
...
When evaluating the above constraint we note that (\ref{dlm:def:restrict:owners}) in \cref{dlm:def:restrict} will always be true, as $owners(L_c)$ is the set of all principals.
